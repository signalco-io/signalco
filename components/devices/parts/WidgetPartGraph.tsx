...
const WidgetPartGraph = ({ columnWidth, config }: { columnWidth: number, config: IWidgetPartGraphConfig }) => {
    ...
    if (config.duration) {
        ...
        const durationRegexMatch = config.duration.match(durationRegex);
    ...
    const CustomTooltip = ({ active, payload }: { active?: boolean, payload?: any }) => {
        if (active && payload && payload.length) {
            const dateTime = domainGraph.invert(payload[0].payload.timeStamp) as Date;
            return (
    ...
    return (
        <Box position="relative">
            <AreaChart
                ...
                <XAxis domain={[0, 1]} ticks={ticks || []} hide dataKey="timeStamp" type="number" />
